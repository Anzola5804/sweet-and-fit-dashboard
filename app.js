...
function switchPage(page) {
    ...
    const titles = { dashboard: 'Dashboard', sales: 'Ventas', inventory: 'Inventario', costs: 'Costos', cobranzas: 'Cobranzas', settings: 'Ajustes' };
...
function renderCobranzas() {
    ...
    state.sales.filter(s => s.status === 'PENDIENTE').forEach(s => {
        const client = s.clientName || 'Cliente Sin Nombre';
        if (!pendingByClient[client]) pendingByClient[client] = [];
        pendingByClient[client].push(s);
    });
    
    const clients = Object.keys(pendingByClient);
    
    if (clients.length === 0) {
        grid.innerHTML = `
            <div style="text-align: center; padding: 40px; color: var(--text-light);">
                <div style="font-size: 48px; margin-bottom: 12px;">✅</div>
                <h3 style="color: var(--aqua); margin-bottom: 8px;">¡Sin deudas pendientes!</h3>
                <p>Todos tus clientes están al día.</p>
            </div>`;
        return;
    }
    
    grid.innerHTML = clients.map(client => {
        const sales = pendingByClient[client];
        const totalDebt = sales.reduce((sum, s) => sum + (s.qty * s.price), 0);
        const totalBs = totalDebt * state.rate;
        
        const ordersList = sales.map(s => {
            const product = state.products.find(p => p.id === s.productId);
            const pName = product ? product.name : 'Producto';
            const date = new Date(s.date).toLocaleDateString('es-VE', { day: '2-digit', month: 'short' });
            const doc = s.documentNumber || 'S/F';
            return `<li>${date} — ${s.qty}x ${pName} ($${(s.qty * s.price).toFixed(2)}) — ${doc}</li>`;
        }).join('');
        
        return `
            <div class="cobranzas-card">
                <div class="cobranzas-header">
                    <div class="cobranzas-client">${client}</div>
                    <div class="cobranzas-amount">$${totalDebt.toFixed(2)}</div>
                </div>
                <div class="cobranzas-bs">Bs ${totalBs.toFixed(2)} · ${sales.length} factura(s)</div>
                <ul class="cobranzas-orders">${ordersList}</ul>
                <button class="btn btn-primary btn-full" onclick="liquidateClientDebt('${client.replace(/'/g, "\\'")}')">💰 Registrar Pago Completo</button>
            </div>`;
    }).join('');
}
